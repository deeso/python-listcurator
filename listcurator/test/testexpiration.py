...
class FixturesTest(unittest.TestCase):
    ...
    def test_listremovekeys(self):
        ...
        test_keys = ['123', 'rtf', 'why', 'expire1', 'expire2']
        test_expire_keys = ['abc', 'xyz', 'expire1', 'expire2']
        test_final_keys = ['123', 'rtf', 'why']
        ...
        rsp5 = a.listexpirekeys(list_name, test_expire_keys)
        has_seen = set()
        ...
        for key in result_dict:
            ...
            if not key in test_keys:
                self.assertFalse(result_dict[key], "Expired a key that did not exist: %s"%key)
        ...
        for key in result_dict:
            ...
            has_seen.add(key)
            if key in test_keys:
                self.assertTrue(result_dict[key], "Did not find a key (%s) the should not be in keys"%key)
            if key in test_expire_keys:
                self.assertTrue(result_dict[key], "Did not find an expired key")

        rsp7 = a.get_basic(list_name).data_string()
        self.assertEqual(rsp7, "\n".join(test_final_keys))

        rsp8 = a.listunexpirekeys(list_name, test_expire_keys)
        has_seen = set()
        result_dict = rsp8['result']
        # print rsp3
        for key in result_dict:
            self.assertFalse(key in has_seen)
            has_seen.add(key)
        ...
        rsp9 = a.get_basic(list_name).data_string()
        self.assertEqual(rsp9, "\n".join(sorted(test_keys)))
