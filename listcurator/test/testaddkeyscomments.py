...
class FixturesTest(unittest.TestCase):
    ...
    def test_listaddkeys(self):
        ...
        test_keys_comments = [('123', 'test1'), ('rtf', 'test2 multiword'), ('why', 'why not have fun?')]
        test_keys_dict = dict([i for i in test_keys_comments])
        ...
        rsp2 = a.listaddkeyscomments(list_name, test_keys_comments)
        has_seen = set()
        ...
        rsp5 = a.get_full(list_name).data_string()
        print rsp5

        input_lines = rsp5.splitlines()
        for i in input_lines:
            key = i.split()[0]
            comment = " ".join(i.split()[2:])
            self.assertTrue(key in test_keys_dict)
            self.assertEqual(test_keys_dict[key], comment)
